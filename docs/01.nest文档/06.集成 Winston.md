---
title: NestJS 集成 Winston 日志
date: 2025-11-05 11:23:42
permalink: /nestjs/winstonIntegration
coverImg: /photos/006.webp
categories:
  - nest
tags:
  - nest
...
`winston-daily-rotate-file` 是 Winston 的一个传输器，用于将日志输出到文件，并按天轮转。<br>
...
import winstonLogger from './config/winston.config'; // [!code ++]

@Module({
  imports: [
    WinstonModule.forRoot({ // [!code ++]
      transports: winstonLogger.transports, // [!code ++]
      format: winstonLogger.format, // [!code ++]
      defaultMeta: winstonLogger.defaultMeta, // [!code ++]
      exitOnError: false, // [!code ++]
    }), // [!code ++]
  ],
  // ...
})
...
```typescript
import { WINSTON_MODULE_NEST_PROVIDER } from "nest-winston"; // [!code ++]

async function bootstrap() {
  const app = await NestFactory.create(AppModule);
  app.useLogger(app.get(WINSTON_MODULE_NEST_PROVIDER)); // [!code ++]
